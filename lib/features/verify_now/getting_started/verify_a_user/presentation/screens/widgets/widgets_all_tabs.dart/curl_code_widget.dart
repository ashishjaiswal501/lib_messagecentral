...
class CurlCodeWidget extends StatelessWidget {
  String sendOtpRequest =
      "curl --location --request POST \'${FlavorConfig.instance!.values!.baseUrl}verification/v3/send?countryCode=${LocalStateCache().countryCode}&customerId=${LocalStateCache().customerId}&flowType=SMS&mobileNumber=${LocalStateCache().phoneNo}\' \\ --header \'authToken: ***********'";
  String sendOtpRequestCopy =
      '''curl --location --request POST '${FlavorConfig.instance!.values!.baseUrl}verification/v3/send?countryCode=${LocalStateCache().countryCode}&customerId=${LocalStateCache().customerId}&flowType=SMS&mobileNumber=${LocalStateCache().phoneNo}' \
  --header 'authToken: ${LocalStateCache().authToken}' ''';
  // String sendOtpRequestCopy2 =
  //     "curl --location --request POST \'${FlavorConfig.instance!.values!.baseUrl}verification/v2/verification/send?countryCode=${LocalStateCache().countryCode}&customerId=${LocalStateCache().customerId}&flowType=SMS&mobileNumber=${LocalStateCache().phoneNo}\' \\ --header \'authToken: ${LocalStateCache().authToken}'";
  ...
  Widget build(BuildContext context) {
    return ListView(
      children: [
        ...
        .sendEvent('verify_now_try_with_code_api_doc', 'verify now try with code api doc', null);
                                    },
